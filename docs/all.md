...
1. 保存用户消息。
2. Leader 把消息分类为 `product_change`。
...
19. 批准保存成功后，由 Leader 根据范围和架构敏感约束选择直接编码或先做系统设计。
20. 简单应用直接把批准后的 `app_spec` 交给 Code Engineer；复杂应用由 Architect 产出并登记
    `system_design` 后，再由 Leader 安排 Code Engineer。
...
11. 工程上下文分为三层：冻结的产品/设计输入、持久化的结构化长记忆、当前工作单元的短记忆。
12. 长记忆只保存有来源的工程事实、已观察文件 hash、已修改文件、完成摘要和未解决问题；
    不保存模型思维链，也不把大段源码复制成长期事实。
13. 短记忆保存最近工具调用和详细结果；最多保留固定窗口，其中只保留最近 5 次文件读写/搜索的
    详细正文，更早记录压缩为摘要和来源元数据。
14. 恢复执行时同时恢复长记忆和短记忆；源码仍是最终事实，长期摘要不能替代重新读取当前文件。

### 2.3 按需检索

1. 已知文件路径时直接 `read_file`；已知类名、函数名或错误文本时先 `search_code`；
   只有不知道代码位置且精确搜索召回不足时，才调用 `retrieve_code_context` 做 RAG 候选召回。
2. 不自动把全仓库目录、接口、类型和相关文件全文塞给模型。模型依据设计、计划和当前记忆，
   主动读取当前步骤需要的最小文件集合。
3. 默认全文读取上限为约 50 KiB 或 1000 行；超过任一阈值必须指定行范围，每次最多约 400 行，
   或先搜索符号再读取命中区间。
4. 实现 `search_code()`：按字符串、路径、函数名检索，返回文件、行号和片段。
5. 大文件且已知目标符号时使用精确搜索和分段读取，不因为文件大就自动使用 RAG。
6. RAG 只负责找到候选文件/片段；命中后仍必须读取当前文件并取得 hash，不能直接按候选片段写入。
7. 这是检索增强，不要求必须用 embedding；首版按当前工作区源码即时切块，使用词项频率、逆文档频率、
   完整短语和路径命中做混合排序，不建立持久化向量索引。
8. 当前实现只检索当前生成应用工作区的受控文本源码；SQL、模板文档和运行日志以后按来源边界分别接入，
   不能直接混入另一个项目的索引。
9. 强制上下文：获批需求 ID、验收条款、工具边界、当前接口合同不能被相似度排名挤掉。
10. 强制上下文按明确身份加载，不能用 RAG 找一份“看起来相似”的旧需求替代。
11. 当前检索来源只有当前应用代码；获批需求、设计和验收合同按明确身份固定加载，不依赖相似度召回。
12. 不能跨项目查别人的源码。
13. 不检索 `.env`、密钥、业务数据库和用户简历。
14. 外部文档只作为参考数据，里面的指令不能提高 Agent 权限。
15. 每段结果带 `project_scope / source_kind / path / content_hash / line_range / retrieval_method / score`。
16. 首版不保存检索缓存，每次调用都重新读取当前工作区并计算 hash，所以修改后不会继续返回旧正文；
    将来若加入索引缓存，必须以工作区代次和内容 hash 做失效控制。
17. 不能让 RAG 片段或未来的检索缓存直接取得写入资格；目标文件仍必须通过 `read_file` 读取最新正文。
18. 上下文首版预算示例：当前模型窗口约 50% 给任务/契约和必要代码、20% 给最新错误/工具结果、10% 给简短工作摘要，留 20% 输出与余量。
19. 实际比例按模型配置。
20. 裁剪必须标注并提供继续读取入口。
21. 摘要只保留可验证事实和来源，不用摘要替代源码或验收记录。
22. 向量检索是后续选项：当当前词法 RAG 在大型陌生仓库上的查询集召回率不足时，
    再考虑 embedding 与精确搜索的混合排序。
23. 是否引入向量库由证据决定；短记忆、固定 PRD、系统设计和已知路径文件不经过 RAG。
24. RAG 验收包含至少 20 个代码定位问题、敏感/跳过目录隔离、无跨工作区泄露、修改后无过期命中。
...
3. 首版工具及工作：
   ...
   - `retrieve_code_context(query, path_filter, max_results)`：只在精确搜索不足后，从当前工作区返回带
     路径、行范围、当前 hash 和评分的候选片段；候选不能替代后续 `read_file`。
   - `write_new_code(path, file_description, expected_hash)`：控制模型决定创建/整体重写一个文件；
     平台在数据库事务外调用专门写码模型生成完整文件，再校验路径、hash 和大小后原子写入。
   - `edit_file_by_replace(path, old_text, new_text, expected_hash)`：对已读取文件做一次精确且唯一的
     局部替换；文件变化、原文缺失或命中多处时拒绝修改，让 Agent 重读。
   - `record_engineering_memory(subject, fact, evidence_paths)`：保存有已读取/已写入文件证据的长期工程事实。
...
11. `write_new_code` 的写码模型只负责目标文件正文，不决定任务顺序、不读取额外文件、不派发后续岗位。
12. 局部修改优先使用 hash 保护的 `edit_file_by_replace`；不能因为替换失败若干次就未经重读自动覆盖整个文件。
...
7. 流程：
   ...
   - `decide_next_action`（模型）
     - 已知路径直接读 / 已知名称精确搜 / 定位不足时 RAG / 读取候选最新源码 /
       记录长期事实 / 生成单文件 / 局部修改 / 检查 → 执行工具 → 记录短期观察 → 再决策
     - 请求验收 → 跑真实验收
       - 失败 → 分类 → 修复上下文 → 再决策
       - 当前单元通过 → 下一个工作单元 → 再检索
       - 全单元完成 → 最终干净验证
...
17. 控制模型负责选择下一动作；`write_new_code` 内部的写码模型只生成一个文件。两次模型调用分别计入执行证据和调用计数。
18. 当前工作单元完成时清空其短记忆，但保留结构化长记忆，供后续文件和恢复执行继续使用。
...
14. 如果数据库没有收到完成记录，但外部动作可能已发生，标 unknown 并 `reconcile_action()` 检查现场：
    - `edit_file_by_replace` 比对前后 hash；已应用返回原结果，不再叠加替换。
