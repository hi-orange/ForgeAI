...
from app.agents.prompts.leader import MESSAGE_CLASSIFICATION_PROMPT_VERSION
...
def require_project_message(db: Session, project_id: int, message_id: int) -> ProjectMessage:
...
def _recent_context(
...
) -> list[leader_agent.ProjectMessageContext]:
    ...
    context: list[leader_agent.ProjectMessageContext] = []
...
def classify_user_message(
    ...
    # 已有结果直接返回，避免重复调用付费模型。
    existing = get_stored_classification(db, message.id)
